...
const getTemp = ({ title, text, img }, rev) => `<div style="display: flex; ${rev ? 'flex-direction: row-reverse' : ''}; align-items: center;">
    ...
    <div className="App">
      <div style={{ width: '90vw', margin: "auto" }}>
        <div style={{ display: "flex" }}>
          <div style={{ width: '20%' }}>title</div><div style={{ width: '40%' }}>text</div><div style={{ width: '40%' }}>image</div>
        </div>
        {data.map((d, index) => {
          return (
            <div key={'index'} style={{ display: "flex" }}>
              <textarea style={{ width: '20%' }} value={d['title']}
                onChange={(evt) => setData((prevState) => {
                  prevState[index]['title'] = evt.target.value
                  return [...prevState]
                })} />
              <textarea style={{ width: '40%' }} value={d['text']}
                onChange={(evt) => setData((prevState) => {
